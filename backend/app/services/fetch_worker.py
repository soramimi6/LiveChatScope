...
from app.services.sqlite_bulk import (
    apply_performance_pragmas,
    disable_messages_fts_triggers,
    ensure_messages_fts_triggers,
    rebuild_messages_fts_index,
)
    ...
    try:
        ...
        for item in chat:
            ...
            if len(batch) >= batch_size:
                if not fts_triggers_disabled:
                    with get_connection() as conn:
                        apply_performance_pragmas(conn)
                        disable_messages_fts_triggers(conn)
                        conn.commit()
                    fts_triggers_disabled = True
                _insert_batch(batch)
                total += len(batch)
                batch.clear()
                if total - last_progress_at >= progress_interval:
        ...
        if fts_triggers_disabled:
            with get_connection() as conn:
                apply_performance_pragmas(conn)
                rebuild_messages_fts_index(conn)
                ensure_messages_fts_triggers(conn)
                conn.commit()
