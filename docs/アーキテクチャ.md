...
> 要件: [概要.md](概要.md) | API: [API仕様.md](API仕様.md) | UI: [UI仕様.md](UI仕様.md)
...
| UI | **shadcn/ui + Tailwind CSS** | 詳細は UI仕様.md |
...
詳細なエンドポイント・スキーマ: [API仕様.md](API仕様.md)
...
| `topic_change_threshold` | **0.35**（[分析パラメータ](#分析パラメータ)） | 変化点判定（cosine 距離） |
...
詳細 DDL・ERD・インデックス: **[DBスキーマ.md](DBスキーマ.md)**  
...
詳細仕様は [API仕様.md](API仕様.md) を参照。ベースパス: **`/api/v1`**。
...
既定値と Stage 別設定は **`backend/config/analysis_defaults.json`** を正とする。  
