...
> **更新**: 2026-06-21 — Phase 0〜3 完了。**次工程・PR 一覧は [工程進捗.md](工程進捗.md)** を正とする。
...
| **実装ルール** | 進捗は [工程進捗.md](工程進捗.md)。本書は項目別の詳細リファレンス |
| **工規模** | **小** = 1 ファイル中心 / **中** = 複数ファイル・API 追加 / **大** = 新パイプライン・横断変更 |
| **Phase 0〜3** | ✅ 実装済（PR #6〜#12） |
...
- [ ] 盛り上がりに配信内位置（UX-09）
- [x] JSON/CSV 役割分担 + ExportMenu 説明（UX-23）
- [ ] ロゴ・ヘッダ（UX-26）

### Phase 3
- [x] スタンプ除外（UX-06）
- [x] グローバルフィルター + refilter（UX-24）
- [x] セッション NG ワード（UX-19）
...
| # | 箇所 | 内容 | 関連 UX |
|---|------|------|---------|
| B-01 | ~~`analysis.py` summary LIMIT~~ | **G-03/UX-08 解消** | — |
| B-02 | `analysis.py` `/topics` | SC count が常に 1 | **D2** |
| B-03 | ~~export vs stage8~~ | **G-04 解消** | — |
| B-04 | ~~`fetch_worker.py` メタ~~ | **G-01 解消** | — |
| B-05 | ~~analyze 即遷移~~ | **G-02 解消** | — |
| B-06 | ~~JSON export~~ | **UX-23 解消** | — |
| B-07 | ~~revenue CSV~~ | **UX-23 解消** | — |
| B-08 | ~~refilter API~~ | **UX-24 解消** | — |
| B-09 | Stage 4 | executemany 済。72k 実測は Phase B | 性能 |
| B-10 | `stream_summary` | summary API は stage7 LIMIT 参照済 | — |

---

## 13. 推奨実装順序（2026-06-21 更新）

```
完了: Phase 0〜3（PR #6〜#12 マージ待ち）
...
| 日付 | 内容 |
|------|------|
| 2026-06-21 | Phase 0〜3 完了反映。§12 既知バグ整理。Phase 4 は工程進捗 §4 へ集約 |
