...
| 項目 | 内容 |
|------|------|
| UX-24 詳細 | ハイブリッド時、フィルター変更で **Stage 4–5 のみ再実行** vs **API 都度集計** のどちらを第一実装にするか（工数・72k 規模で要検証） |
...
#### UX-24: グローバル表示フィルター 📋 ✅方針確定（Q1: ハイブリッド）
...
| 対象 | フィルター | 備考 |
|------|:----------:|------|
| 密度・盛り上がり・Top 投稿者（件数） | **OFF** | チャット全体の熱量として維持 |
| 話題ブロック・キーワード | **ON** | UX-06 の text_only と整合 |
| サマリー KPI（総コメント数等） | **要検討** | 全件のまま vs フィルター後を併記 |
| JSON export | **ON**（messages + 派生） | JSON は「全部」だが messages / 話題系はフィルター反映 |
| CSV export | **ON** | messages 専用のためフィルター行のみ出力 |
| Markdown export | **ON**（話題・キーワード系） | 密度・clips は全件のまま |

| **実装メモ** | `video-dashboard` Context。フィルター変更時は **Stage 4–5 再実行** または **API 都度集計**（§1 残未決）。NG ワードは UX-19 と統合 |
| **触るファイル** | `summary-tab.tsx`, `video-dashboard.tsx`, `export.py`, `stage4.py`, `stage5.py`, 各 API |
...
- [ ] UX-24: ハイブリッドフィルター（密度は全件・話題系は除外）
- [ ] UX-25 Must: D2, B1, C1
