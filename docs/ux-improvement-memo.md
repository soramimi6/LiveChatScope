...
| **Q6** | UX-05 | 取得結果から **判断可能なら UI で区別**すべき（「SC なし配信」vs「取得・解析上の問題」） |
| **Q7** | UX-24 | フィルター変更時は **A: Stage 4–5（＋派生）の Pipeline 再実行**。待ち時間が発生するため **「更新中」UI** を表示（スピナー、対象タブの無効化等） |
...
| UX-24 連鎖範囲 | Stage 4–5 再実行時に Stage 6–7（話題遷移・summary）まで **連鎖再実行**するか（推奨: する） |
