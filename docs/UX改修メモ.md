...
> **関連**: [第一弾チェックリスト.md](第一弾チェックリスト.md) §C / **[実施計画](UX実施計画.md)** / **[工程進捗](工程進捗.md)**
...
| **方針判断** | 分析・調査ツールとして **正確性が最優先**。実装コストより **データ・画面の一貫性** を優先し、自明な選択肢は確認せずメモに確定方針として記載する |
...
| 進捗 → 結果遷移 | 分析完了待ち（[E2E手順書.md](E2E手順書.md)） | `fetch_status=fetched` で即 `/videos/` へ（`analyze/…/page.tsx` L33–35） | 分析 `running` 中に結果画面へ行き API 409 の可能性 |
...
**触るファイル**: `export.py`, `export-menu.tsx`, `revenue-tab.tsx`, `docs/要件定義.md`, E2E テスト
