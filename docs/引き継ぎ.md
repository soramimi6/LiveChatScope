...
### UX 改修 PR スタック（`dev` 未マージ — 2026-06-21）

マージ順: **#6 → #7 → #8 → #9 → #10 → #11 → #12**

| PR | ブランチ | 内容 |
|:--:|----------|------|
| #6 | `cursor/ux-phase0-phase1-10cc` | Phase 0/1: G-02, UX-01/02/04/08/13/22/27 |
| #7 | `cursor/ux-g01-video-meta-10cc` | G-01 動画メタ |
| #8 | `cursor/ux-g05-superchat-status-10cc` | UX-05 SC 0 件判別 |
| #9 | `cursor/ux-g04-markdown-unify-10cc` | G-04 markdown 統一 |
| #10 | `cursor/ux-23-json-csv-export-10cc` | UX-23 JSON/CSV |
| #11 | `cursor/ux-06-24-global-filter-10cc` | UX-06/24 フィルター |
| #12 | `cursor/ux-19-session-ng-words-10cc` | UX-19 セッション NG |

`dev` のみ: **UX-21** エクスポートファイル名（`f6eaa6b`）

進捗・Phase 4 予定: **[工程進捗.md](工程進捗.md)**
...
app/
  ...
  api/
    ...
    analysis.py        # summary, topics, POST …/analysis/refilter
  ...
  services/
    ...
    analysis/          # pipeline, refilter_pipeline, message_filter, stage0–8
db/schema.sql
...
lib/
  api.ts               # videos, summary, display_filter, refilter
...
| 設計 vs 実装の差分 | [UX改修.md §2](UX改修.md) — Phase 4 残: D2 SC count、UX-07 TL 等 |
...
- [ ] 本ファイル + [工程進捗.md §4](工程進捗.md) を読んで Phase 4 着手

---

## 11. 次工程

**Phase 4 差別化 Must**（D2 → B1 → C1）— 詳細は **[工程進捗.md §4](工程進捗.md)**。

| 順 | ID | 概要 |
|:--:|-----|------|
| 1 | **D2** | SC × 話題ブロック（`/topics` SC count バグ修正 + 可視化） |
| 2 | **B1** | キーワード急上昇（burst API + UI） |
| 3 | **C1** | 常連コア層プロファイル（author profile API + Sheet） |

**前提**: PR #6〜#12 を `dev` にマージしてから着手。
